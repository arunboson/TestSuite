...
def getMotorTempbytes(MotorTemp):
    ...
    data[6] = numpy.int8(newMotorTemp)
    data[7] = numpy.int8(newMotorTemp>>8)
    return data
...
def capVoltageBytes(capVolts):
    ...
    data[0] = numpy.int8(newCapVolts)
    data[1] = numpy.int8(newCapVolts>>8)
    return data 
...
def batt_i_Bytes(batt_i):
    ...
    data[4] = numpy.int8(batt_i_16bit)
    data[5] = numpy.int8(batt_i_16bit>>8)
    return data 
...
def getRPMbytes(RPM):
    ...
    data[4] = numpy.int8(newRPM)
    data[5] = numpy.int8(newRPM>>8)
    data[6] = numpy.int8(newRPM>>16)
    data[7] = numpy.int8(newRPM>>24)
    return data
